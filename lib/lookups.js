...
var lookups = {
  ...
  addBrewery: function(name, address, city) {
    return breweries.insert({ name: name, address: address, city: city });
  },
  breweryName: function(name) {
    return breweries.find({ name: name });
  },
  editBrewery: function(id, name, address, city) {
    return breweries.updateById(id, { name: name, address: address, city: city });
  },
  deleteBrewery: function(id) {
    return breweries.findById(id).then(function(brewery) {
        beers.find({ breweryId: brewery._id }).then(function(allBeers) {
          ...
          beers.remove({ breweryId: brewery._id });
        })
      breweries.remove({ _id: brewery._id });
    })
  },
  getBeers: function(breweryId) {
    return beers.find({ breweryId: breweryId }, {sort: {name: 1}});
  },
  getBeersTap: function(beerIds) {
    return beers.find({ _id: beerIds},, {sort: {name: 1}});
  }
  addBeer: function(breweryId, name, style, description, alcohol) {
    return breweries.findById(breweryId).then(function(brewery) {
      ...
      beers.insert({ name: name, style: style, description: description, alcohol: alcohol, breweryId: brewery._id, tapIds: [] });
    })
  },
